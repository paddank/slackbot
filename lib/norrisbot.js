...
var NorrisBot = function Constructor(settings) {
    console.log('constructing', settings.name, this.name)
...
NorrisBot.prototype.run = function () {
    console.log('run pre super')
    NorrisBot.super_.call(this, this.settings);
    console.log('run post super')
...
NorrisBot.prototype._replyWithRandomJoke = function (originalMessage) {
    var self = this;
    var stmt = null;
    var topic = self._hasTopic(originalMessage.text);

    if (topic) {
        topic = topic.replace(/\W/g, '')  // strip non alpha/numeric chars
        stmt = self.db.prepare('SELECT joke FROM jokes WHERE joke LIKE ?', '%'+topic+'%');
    } 
    else {
        stmt = self.db.prepare('SELECT id, joke FROM jokes ORDER BY used ASC, RANDOM() LIMIT 1');
    }

    if (! stmt) {
        return console.error("STATEMENT ERROR");
    }

    stmt.get(function (err, record) {
        if (err) {
        ...
        if (! record) {
            console.log("no matching joke found on topic:", topic)
            self.postMessageToChannel(channel.name, "I dunno, I only deal with death!", {as_user: true});
        }
        else {
            console.log("it's funny time: "+record.joke)
...
NorrisBot.prototype._loadBotUser = function () {
    this.user = this.self
    this.user.name = this.settings.name || this.self.name
};
...
NorrisBot.prototype._isMentioningChuckNorris = function (message) {
    return message.text.toLowerCase().indexOf('chuck') > -1 ||
           message.text.toLowerCase().indexOf('norris') > -1 ||
           message.text.toLowerCase().indexOf('chuckie') > -1 ||
...
NorrisBot.prototype._hasTopic = function (message) {
    var topic = null

    // break down string to array of words
    var words = message.split(' ')

    // find index of 'about' in words (if exists)
    for (var n = 0; n < words.length; n++) {
        if (words[n].toLowerCase() === 'about') {
            // check if there is another word coming after 'about' and make it the topic
            if (n+1 < words.length) {
                topic = words[n+1]
            }
        }
